...
import { getAdminEnvState } from "@/lib/admin/getAdminAccessState";

export const dynamic = "force-dynamic";

export default async function AdminLayout({ children }: { children: ReactNode }) {
  const state = await getAdminEnvState();
      ...
      {!state.ok ? (
        <div className="mx-auto mt-6 max-w-4xl" role="alert">
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
            <p className="font-semibold text-amber-900">Admin — configuration required</p>
