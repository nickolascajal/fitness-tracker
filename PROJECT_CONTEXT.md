...
* Next Session Focus recommendation copy is shorter and more direct while preserving the same progression triggers and calculation logic, including explicit target load text for increase steps (`highest completed working weight + increment`)
...
- **Routes:**
  ...
  - `/admin/exercises` — **Exercise Analytics Database** (read-only): cross-user aggregates grouped by `exerciseNameKey` (`lib/exerciseNameKey.ts`); each row expands to show distinct exercise configs (from `exercises.data`) with user counts, session counts, CPS high/low/average from completed non-draft workouts (`workouts.data`, matched by `exerciseId`, with name+config fallback only when a single config matches that user). **Analytics-only outlier filtering** (`lib/admin/exerciseAnalyticsOutliers.ts`) excludes impossible or extreme values from aggregate CPS stats (conservative absolute caps; optional median-based high cap per exercise+config when enough samples exist) while **preserving original user workout data** and user-facing CPS. The UI shows included vs excluded session counts. No individual user identities or emails on this page.
...
- **Data fetching (server-only):**
  ...
  - Admin user detail (`/admin/user/[userId]`) now has a unified **Add Workout for User** flow with mode selection: **Assign as planned workout** creates `isDraft: true` rows with empty inputs for later completion, while **Add as completed historical workout** creates `isDraft: false` rows with provided set data and server-side calculated CPS/progression/recommendation, all inserted under the target `user_id`.
  - Admin can manage target-user calendar setup from `/admin/user/[userId]` by marking/unmarking rest days and assigning planned draft workouts with optional prefilled inputs (for example, prefilled weight with blank reps). Rest-day markers and workout rows are stored under the target `user_id`; rest-day marking is blocked when workouts already exist on that date.
  - Admin user detail (`/admin/user/[userId]`) now centers on a calendar-based day workflow: admins select a date, see day overview status (draft/planned, completed, rest-day), add single exercises (existing user config or quick config) or presets, edit planned/completed workout set data, delete workout rows with confirmation, and manage rest-day toggles. All writes remain server-validated and scoped to the target route `user_id` (never admin `user_id`).
  - Admin exercise/preset creation flows support three exercise sources: **target user exercise configs**, **master exercise list (`EXERCISES_BY_LETTER`)**, or **custom typed names**. Both single-exercise add and preset-add controls include compact **search by name** and **type filter** (`all`, `weight`, `bodyweight`, `time`). Master list entries are never mutated; missing configs are created only under the target `user_id`.
